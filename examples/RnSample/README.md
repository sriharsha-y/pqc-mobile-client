...
- `android/app/src/main/java/com/rnsample/PqcInterceptor.kt` is a thin `RnSamplePqcInterceptor` subclass of the bundled `io.github.sriharsha_y.pqc.PqcInterceptor` — overrides `makeConfig()` for the sample's banking-style posture and adds the `X-Pqc-Mode: off` toggle. Must be the **last** interceptor; later ones never fire because the Rust core terminates the chain.
...
- `ios/RnSample/PqcURLProtocol.swift` is `RnSamplePqcURLProtocol`, a thin subclass of the bundled `PqcCore.PqcURLProtocol`. Overrides `makeConfig()` for the sample's banking-style posture and `shouldHandle(_:)` for the `X-Pqc-Mode: off` toggle. Sample intercepts every `https://`; a real app should narrow this to specific hostnames.
- `ios/RnSample/AppDelegate.mm` calls `RCTSetCustomNSURLSessionConfigurationProvider(...)` from `didFinishLaunchingWithOptions`. The provider returns a `URLSessionConfiguration` with `[RnSamplePqcURLProtocol class]` prepended to `protocolClasses` — *except* on iOS 26+ where the native URLSession already negotiates PQC.
