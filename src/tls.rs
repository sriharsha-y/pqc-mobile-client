...
use crate::pinning::{decode_domain_pins, PinningVerifier};
...
///   continue to work.
/// - When `pinned_domains` is non-empty, wraps the platform verifier in a
///   `PinningVerifier` that additionally enforces a per-host SPKI pin from the
///   chain (hosts with no matching entry stay unpinned).
pub fn build_tls_config(cfg: &PqcConfig) -> Result<ClientConfig, PqcError> {
    ...
    let tls = if cfg.pinned_domains.is_empty() {
        ...
        let domains = decode_domain_pins(&cfg.pinned_domains)?;
        let pinning = Arc::new(PinningVerifier::new(inner, domains));
